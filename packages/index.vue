<template>
  <div class="workflow-top">
    <el-button size="small" :disabled="currentComponentIndex <= 0" @click="undo">撤销</el-button>
    <el-button size="small" :disabled="currentComponentIndex >= cacheComponentList.length - 1" @click="redo">恢复
    </el-button>
    <el-button :type="lineType === 'straight' ? 'primary' : 'default'" size="small" @click="changelineType('straight')">
      直线</el-button>
    <el-button :type="lineType === 'broken' ? 'primary' : 'default'" size="small" @click="changelineType('broken')">折线
    </el-button>
    <el-button :type="lineType === 'bezier' ? 'primary' : 'default'" size="small" @click="changelineType('bezier')">曲线
    </el-button>
    <el-button size="small" @click="clearCanvas">清空画布</el-button>
    <slot name="btn" />
  </div>
  <div class="workflow-main" :style="{ height: mainHeight }">
    <div class="workflow-left">
      <el-scrollbar style="padding: 10px;">
        <div v-for="(component, index) in componentList" :key="component.id" class="component-item" draggable="true"
          @dragstart="dragstart($event, index)">
          <slot v-if="$slots.left" :component="component" :index="index" name="left" />
          <div v-else>
    ...
    <div ref="middleRef" class="workflow-middle">
      <div ref="maskRef" class="mask" :style="{
        transform: `translate(${left}px, ${top}px)`
      }" @dragover="dragover" @drop="ondrop">
        <canvas id="mask-canvas" ref="maskCanvas"></canvas>
        <div class="workflow-container" data-type="container"
          :style="{ width: width, height: height, cursor: canvasCursor }" @mousedown="containerDown"
          @contextmenu="customMenu" @mousemove.self="canvasMove" @click.self="maskClick" @dblclick.self="maskDbClick">
          <div v-for="(component, index) in componentRenderList" :key="component.id"
            :class="['render-component', isSelectedComponent(component.id) && 'active']" :style="{
            ...
            <div class="shape-point up" :style="{
              ...
              @mouseup.stop="drawLineEnd($event, 'up', component)" @mouseover.self="drawLineOver('up', component)"
              @mouseleave.self="drawLineLeave"></div>
            <div class="shape-point right" :style="{
              left: component.attr.w + 'px',
              top: component.attr.h * 0.5 + 'px'
            }" @mousedown.stop="drawLineStart($event, 'right', component)"
              @mouseup.stop="drawLineEnd($event, 'right', component)" @mouseover.self="drawLineOver('right', component)"
              @mouseleave.self="drawLineLeave"></div>
            <div class="shape-point down" :style="{
              left: component.attr.w * 0.5 + 'px',
              top: component.attr.h + 'px'
            }" @mousedown.stop="drawLineStart($event, 'down', component)"
              @mouseup.stop="drawLineEnd($event, 'down', component)" @mouseover.self="drawLineOver('down', component)"
              @mouseleave.self="drawLineLeave"></div>
            <div class="shape-point left" :style="{
              left: 0,
              top: component.attr.h * 0.5 + 'px'
            }" @mousedown.stop="drawLineStart($event, 'left', component)"
              @mouseup.stop="drawLineEnd($event, 'left', component)" @mouseover.self="drawLineOver('left', component)"
              @mouseleave.self="drawLineLeave"></div>
          </div>
          <!-- 组件拉伸点 -->
          <div v-if="activeComponet && areaPoint" class="shape-control" :style="{
            left: areaPoint.x + 'px',
            top: areaPoint.y + 'px',
          ...
          <!-- 所有的线文本展示 -->
          <div v-for="extra in lineExtraInfos" :key="extra.id" class="line-extrainfo" :style="{
            left: extra.point[0] + 'px',
            top: extra.point[1] + 'px'
          }" @dblclick.stop="editExtraInfo(extra)">{{ extra.text }}</div>
          <!-- 线文本信息编辑 -->
          <div v-if="editExtraInfoStatus" ref="editExtraInfoRef" class="line-extrainfo editing" contenteditable :style="{
            left: activeExtraInfo?.point[0] + 'px',
            top: activeExtraInfo?.point[1] + 'px'
          }" @blur="editExtraInfoBlur" @keydown.enter="editExtraInfoBlur" v-text="activeExtraInfo?.text"></div>
          <!-- 范围选择 -->
          <div v-show="areaSelect" class="area-contaier" :style="{
            left: areaPosi?.x + 'px',
            top: areaPosi?.y + 'px',
            width: areaPosi?.w + 'px',
          ...
          <!-- 吸附线 -->
          <div v-show="adsordPoint[1] !== null" class="adsorb-line coord-x" :style="{
            top: adsordPoint[1] + 'px'
          }"></div>
          <div v-show="adsordPoint[0] !== null" class="adsorb-line coord-y" :style="{
    ...
    <div class="workflow-right">
      <el-scrollbar style="padding: 10px;" v-if="properties">
        <slot v-if="$slots.right" :properties="properties" :title="activeComponet?.label || 'Line'" name="right" />
        <div v-else>
          ...
          <el-form label-position="top">
            <el-form-item v-for="copProp in properties" :key="copProp.name" :label="copProp.label || copProp.name"
              ...
              <el-input v-if="copProp.type === 'input'" v-model="copProp.value" clearable
                :disabled="copProp.disabled && operateType === 'edit'" v-bind="copProp.props || {}">
              </el-input>
              <el-select v-else-if="copProp.type === 'select'" v-model="copProp.value" clearable filter
                :disabled="copProp.disabled && operateType === 'edit'" v-bind="copProp.props || {}">
                <el-option v-for="opt in propOptions?.[copProp.name]" :key="opt.value" :label="opt.label"
                  :value="opt.value">
                </el-option>
              </el-select>
            </el-form-item>
